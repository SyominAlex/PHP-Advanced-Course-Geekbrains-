...
class App
{
    ...
    // Будем хранить компоненты с отложенной инициализацией
    public $components;
    ...
    // Магический метод __get, который будет вызываться при обращении к свойству, которого нет в нашем массиве components конфига
    function __get($name)
    {
    ...
    function createComponent($name)
    {
        if (isset($this->config['components'][$name])) {
            ...
            if (class_exists($className)) {
                ...
                // ReflectionClass::newInstanceArgs создает новый экземпляр класса, принятые аргументы передаются в конструктор класса
